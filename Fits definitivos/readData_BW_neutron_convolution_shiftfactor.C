...
#include <math.h>
#include <TLegend.h>
#include "binContents_neutron_60.C"
#include "binContents_neutron_10000.C"
...
double BWModificada(double *x, double *par, int l, int bw_index) {
    static const double E_min_values[9] = {11.46, 11.46, 11.46, 11.46, 11.46, 11.46, 11.46, 11.46, 11.46};
    static const double E_max_values[9] = {14.35, 14.35, 14.35, 14.35, 14.35, 14.35, 14.35, 14.35, 14.35};

    if (bw_index < 0 || bw_index >= 9) return 0.0;
    ...
    int bin_index_bw = (E - E_min_values[bw_index]) / 0.000289;
    ...
    double E_bin = E_min_values[bw_index] + (bin_index_bw + 0.5) * 0.000289;
    double Gamma_eff;
    double E_eff;
    ...
    if (E_bin >= 11.46) {
        ...
        double E_pen_l0 = -Gamma0 * ((T0_neutron_values[bin_index_pen]-T0_neutron_values[bin_index_E0])/(2*T0_neutron_values[bin_index_E0]));
        double E_pen_l1 = -Gamma0 * ((T1_neutron_values[bin_index_pen]-T1_neutron_values[bin_index_E0])/(2*T1_neutron_values[bin_index_E0]));
        double E_pen_l2 = -Gamma0 * ((T2_neutron_values[bin_index_pen]-T2_neutron_values[bin_index_E0])/(2*T2_neutron_values[bin_index_E0]));
        double E_pen_l3 = -Gamma0 * ((T3_neutron_values[bin_index_pen]-T3_neutron_values[bin_index_E0])/(2*T3_neutron_values[bin_index_E0]));
        double E_pen_l4 = -Gamma0 * ((T4_neutron_values[bin_index_pen]-T4_neutron_values[bin_index_E0])/(2*T4_neutron_values[bin_index_E0]));
        double E_pen_l5 = -Gamma0 * ((T5_neutron_values[bin_index_pen]-T5_neutron_values[bin_index_E0])/(2*T5_neutron_values[bin_index_E0]));
...
double TotalFunction(double *x, double *par) {
    ...
    double total = 0.0;
    const int numBW = 9;
    int l_values[9] = {0, 1, 0, 2, 1, 0, 0, 0, 2};
...
void readData_BW_neutron_convolution_shiftfactor()
{
...
}, 11.46, 14.35, 4);  // E_min = 11.2, E_max = 12.0, número de parámetros = 4

TF1 *bw2 = new TF1("bw2", [=](double *x, double *par) { 
    return BWModificada(x, par, 1, 1); 
}, 11.46, 14.35, 4);  // E_min = 11.5, E_max = 12.5, número de parámetros = 4
...
}, 11.46, 14.35, 4);  // E_min = 12.0, E_max = 13.0, número de parámetros = 4
...
}, 11.46, 14.35, 4);  // E_min = 13.7, E_max = 14.5, número de parámetros = 4
...
bw8->SetParameters(10, 13.6, 0.3, 0.0657);
bw9->SetParameters(10, 14, 0.3, 0.0657);

// Crear la función total (sumatoria de las funciones Breit-Wigner)
TF1 *total = new TF1("mstotal", TotalFunction, 11.46, 14.35, 36);
...
total->SetParLimits(20, 1, 10000); 
total->FixParameter(21, 13.235); 
total->FixParameter(22, 0.25); 
total->FixParameter(23, 0.0657);

total->SetParLimits(24, 1, 10000); 
total->FixParameter(25, 13.613); 
total->FixParameter(26, 0.254);  
total->FixParameter(27, 0.0657);

total->SetParLimits(28, 1, 10000); 
total->FixParameter(29, 13.9); 
total->FixParameter(30, 0.216);  
total->FixParameter(31, 0.0657);

total->SetParLimits(32, 1, 10000); 
total->FixParameter(33, 14.1); 
total->FixParameter(34, 0.140); 
total->FixParameter(35, 0.0657);
...
double fwhm1 = calculateFWHM(new_bw1, par_total[1], 11.46, 14.35);
double fwhm2 = calculateFWHM(new_bw2, par_total[5], 11.46, 14.35);
double fwhm3 = calculateFWHM(new_bw3, par_total[9], 11.46, 14.35);
double fwhm4 = calculateFWHM(new_bw4, par_total[13], 11.46, 14.35);
double fwhm5 = calculateFWHM(new_bw5, par_total[17], 11.46, 14.35);
double fwhm6 = calculateFWHM(new_bw6, par_total[21], 11.46, 14.35);
double fwhm7 = calculateFWHM(new_bw7, par_total[25], 11.46, 14.35);
double fwhm8 = calculateFWHM(new_bw8, par_total[29], 11.46, 14.35);
double fwhm9 = calculateFWHM(new_bw9, par_total[33], 11.46, 14.35);
...
Double_t integral_bw3 = new_bw3->Integral(11, 15) *15;
Double_t integral_bw4 = new_bw4->Integral(11, 15) *15;
Double_t integral_bw5 = new_bw5->Integral(11, 15) *15;
Double_t integral_bw6 = new_bw6->Integral(11, 15) *15;
Double_t integral_bw7 = new_bw7->Integral(11, 15) *15;
Double_t integral_bw8 = new_bw8->Integral(11, 15) *15;
Double_t integral_bw9 = new_bw9->Integral(11, 15) *15;
