...
double BWModificada(double *x, double *par, int l, int bw_index) {
  static const double E_min_values[6] = {11.23, 11.23, 11.23, 11.23, 11.23, 11.23};
  ...
  if (E_bin >= 11.23) {
    ...
    double Gamma_pen_l2 = Gamma0 * T2_values[bin_index_pen] / T2_values[bin_index_E0];
    double Gamma_pen_l3 = Gamma0 * T3_values[bin_index_pen] / T3_values[bin_index_E0];
    ...
    double E_pen_l0 = -Gamma0 * ((T0_values[bin_index_pen]-T0_values[bin_index_E0])/(2*T0_values[bin_index_E0]));
    double E_pen_l1 = -Gamma0 * ((T1_values[bin_index_pen]-T1_values[bin_index_E0])/(2*T1_values[bin_index_E0]));
    double E_pen_l2 = -Gamma0 * ((T2_values[bin_index_pen]-T2_values[bin_index_E0])/(2*T2_values[bin_index_E0]));
    double E_pen_l3 = -Gamma0 * ((T3_values[bin_index_pen]-T3_values[bin_index_E0])/(2*T3_values[bin_index_E0]));
    double E_pen_l4 = -Gamma0 * ((T4_values[bin_index_pen]-T4_values[bin_index_E0])/(2*T4_values[bin_index_E0]));
    double E_pen_l5 = -Gamma0 * ((T5_values[bin_index_pen]-T5_values[bin_index_E0])/(2*T5_values[bin_index_E0]));
...
void readData_BW_proton_penetrability_shiftfactor() {
  ...
  TCanvas *c2 = new TCanvas("c2", "Plot Penetrabilidad Proton", 800, 600);
  ...
  TH1F *exTotalH = new TH1F("exTotalH", "exTotalH", 22, 11, 14.5);
...
// 15 bines
/*
double binEfficiency10k[] = {1, 1, 0.15, 0.1634, 0.1622, 0.1510, 0.147, 0.1431, 0.137, 0.1317, 0.127, 0.1231, 0.1164, 0.113, 0.1106, 0.1036, 0.097, 0.0938, 0.0900, 0.087, 0.0821, 0.0756};
*/

double binEfficiency10k[] = {1, 1, 0.1339, 0.1603, 0.16, 0.1543, 0.1485, 0.1435, 0.1377, 0.1298, 0.1244, 0.1197, 0.1129, 0.1109, 0.1079, 0.103, 0.0957, 0.0920, 0.0861, 0.0827, 0.0774, 0.0780, 0.0757};
...
/*
  ...
  for (int i = 1; i <= exTotalH->GetNbinsX(); i++) {
    ...
    if (binCenter >= 11 && binCenter <= 14.5) {
      int efficiencyIndex =
          static_cast<int>((binCenter - 11) / (14.5 - 11) * 22);
      if (efficiencyIndex >= 0 && efficiencyIndex < 22) {
        double efficiency = binEfficiency10k[efficiencyIndex];
        if (efficiency > 0) {
          double content = exTotalH->GetBinContent(i);
          double error = exTotalH->GetBinError(i);
          exTotalH->SetBinContent(i, content / efficiency);
          exTotalH->SetBinContent(i, error/efficiency);
        }
      }
    }
  }
...
bw6->SetParameters(50, 14, 0.2, 0.0657);  

// Definir total dada por la suma de los 16 picos
TF1 *total = new TF1("mstotal", TotalFunction, 11., 14.5, 24);
...
total->SetParLimits(0, 1, 100);
total->SetParLimits(1, 11.43, 11.46);
total->SetParLimits(2, 0.015, 0.05);
total->FixParameter(3, 0.0657);
total->SetParLimits(4, 1, 100);
total->SetParLimits(5, 12.1, 12.12);
total->SetParLimits(6, 0.24, 0.36);
total->FixParameter(7, 0.0657);
total->SetParLimits(8, 1.75, 100);
total->SetParLimits(9, 12.5, 12.60);
total->SetParLimits(10, 0.27, 0.29);
total->FixParameter(11, 0.0657);
total->SetParLimits(12, 1, 100);
total->SetParLimits(13, 13.24, 13.27);
total->SetParLimits(14, 0.230, 0.250);
total->FixParameter(15, 0.0657);
total->SetParLimits(16, 3.5, 100);
total->SetParLimits(17, 13.57, 13.64);
total->SetParLimits(18, 0.225, 0.24);
total->FixParameter(19, 0.0657);
total->SetParLimits(20, 1, 100);
total->SetParLimits(21, 13.89, 13.93);
...
TF1 *new_bw1 = new TF1("new_bw1", [=](double *x, double *par_total) {
  return BWModificada(x, par_total, 2, 0);
}, 11.23, 14.5, 4);
...
// Establecer los parámetros de las funciones new_bw
new_bw1->SetParameters(&par_total[0]);
...
double fwhm1 = calculateFWHM(new_bw1, par_total[1], 11.23, 14.5);
double fwhm2 = calculateFWHM(new_bw2, par_total[5], 11.23, 14.5);
double fwhm3 = calculateFWHM(new_bw3, par_total[9], 11.23, 14.5);
double fwhm4 = calculateFWHM(new_bw4, par_total[13], 11.23, 14.5);
double fwhm5 = calculateFWHM(new_bw5, par_total[17], 11.23, 14.5);
double fwhm6 = calculateFWHM(new_bw6, par_total[21], 11.23, 14.5);
...
/*
Double_t integral_bw1 = new_bw1->Integral(11, 14.5) / ((14.5-11)/22);
cout << "Integral of new_bw1: " << integral_bw1 << endl;


Double_t integral_bw2 = new_bw2->Integral(11, 14.5) / ((14.5-11)/22);
Double_t integral_bw3 = new_bw3->Integral(11, 14.5) / ((14.5-11)/22);
Double_t integral_bw4 = new_bw4->Integral(11, 14.5) / ((14.5-11)/22);
Double_t integral_bw5 = new_bw5->Integral(11, 14.5) / ((14.5-11)/22);
Double_t integral_bw6 = new_bw6->Integral(11, 14.5) / ((14.5-11)/22);
...
/*
TFile *outfile = TFile::Open("Plot_Penetrabilidad_Proton.root", "RECREATE");
c2->Write();
exTotalH->Write(); 
outfile->Close();
*/
