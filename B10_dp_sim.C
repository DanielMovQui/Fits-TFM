void B10_dp_sim(Int_t nEvents = 1000, TString mcEngine = "TGeant4")
{
...
/*
 ...
 // ------------------------------------------------------------------------
  ...
  FairPrimaryGenerator* primGen = new FairPrimaryGenerator();
	        ...
	        Int_t m = 1;   // Multiplicity  NOTE: Due the limitation of the TGenPhaseSpace accepting only pointers/arrays the maximum multiplicity has been set to 10 particles. //!  POR MODIFICAR
		 ...
		 // Variables for 2-Body kinematics reaction
		  ...
		  // Note: Momentum will be calculated from the phase Space according to the residual energy of the beam
	          ...
	          mult = 4; //Number of Nuclei involved in the reaction (Should be always 4) THIS DEFINITION IS MANDATORY (and the number of particles must be the same)
                  ResEner = 140.0; // For fixed target mode (Si Array) in MeV //!  POR MODIFICAR
		 ...
		 ExE.push_back(0.0);//In MeV
          	...
          	ExE.push_back(15); //! Distribucion de energia o Sn?
		 ...
		 ExE.push_back(0.0);//! nada porque es proton en estado fundamental?


                 Double_t ThetaMinCMS = 10.0;
                 Double_t ThetaMaxCMS = 70.0;
...
// 10B
zDecay.at(0).push_back(5);
aDecay.at(0).push_back(10);
qDecay.at(0).push_back(0);
massDecay.at(0).push_back(10.0129370); // masa de 10B en u

// neutrón
zDecay.at(0).push_back(0);
aDecay.at(0).push_back(1);
qDecay.at(0).push_back(0);
massDecay.at(0).push_back(1.00866491595);
