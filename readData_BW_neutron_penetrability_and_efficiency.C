...
double binContents[] = {
        0.0112196,    0.00447391,    0.0156728,    0.0398542,    0.14215,    0.188717,
    0.412943,    0.668679,    0.835731,    1.02656,    1.19251,    1.43721,    1.67934,    1.96693,
    1.9544,    2.11799,    2.09751,    2.00633,    2.32176,    2.20975,    2.33303,    2.73834,
    2.45824,    2.74923,    2.75967,    2.7088,    2.79933,    2.9985,    3.15739,    3.04868,
    3.01255,    3.14351,    3.15208,    3.57383,    3.68672,    3.47688,    3.14009,    2.96725,
    3.36487,    3.27668,    3.35067,    3.28964,    3.48007,    3.30473,    3.08121,    3.74655,
    3.53505,    3.11932,    3.49181,    3.93856,    3.03112,    3.30352,    3.59637,    3.27146,    3.24624,    3.23586,
    3.09021,    3.04928,    3.75548,    3.20746
};
/*
...
double BWModificada(double *x, double *par, int l, int bw_index) {
    ...
    // Definir los rangos de energía para cada Breit-Wigner (E_min y E_max)
        static const double E_min_values[10] = {11, 11, 11, 11, 11, 11, 11, 11, 11, 11};  // Energías mínimas
        static const double E_max_values[10] = {15, 15, 15, 15, 15, 15, 15, 15, 15, 15};  // Energías máximas
...
double TotalFunction(double *x, double *par) {
    ...
    const int numBW = 10;
    int l_values[10] = {0, 1, 0, 2, 3, 0, 0, 5, 0, 5}; 
...
void readData_BW_neutron_penetrability_and_efficiency()
{
...
}, 11, 15, 5);  // E_min = 12.5, E_max = 14.0, número de parámetros = 4
...
bw8->SetParameters(1000, 13.8, 0.3, r, 0.0657);
bw9->SetParameters(1000, 14, 0.3, r, 0.0657);
bw10->SetParameters(1000, 14.3, 0.3, r, 0.0657);

// Crear la función total (sumatoria de las funciones Breit-Wigner)
TF1 *total = new TF1("mstotal", TotalFunction, 11, 15, 50);
...
total->SetParLimits(0, 240, 10000); 
total->SetParLimits(1, 11.58, 11.67); 
total->SetParLimits(2, 0.18, 0.25);
total->FixParameter(3, 8.5e-11); 
total->FixParameter(4, 0.0657);

total->SetParLimits(5, 350, 10000); 
total->SetParLimits(6, 11.81, 11.84); 
total->SetParLimits(7, 0.24, 0.3);    
total->FixParameter(8, 8.5e-11);
total->FixParameter(9, 0.0657);

total->SetParLimits(10, 1, 10000); 
total->SetParLimits(11, 11.95, 12.05); 
total->SetParLimits(12, 0.25, 0.5);   
total->FixParameter(13, 8.5e-11);
total->FixParameter(14, 0.0657);

total->SetParLimits(15, 1400, 10000); 
total->SetParLimits(16, 12.48, 12.52); 
total->SetParLimits(17, 0.5, 0.8);  
total->FixParameter(18, 8.5e-11);
total->FixParameter(19, 0.0657);

total->SetParLimits(20, 50, 10000); 
total->SetParLimits(21, 12.85, 12.95); 
total->SetParLimits(22, 0.2, 0.3); 
total->FixParameter(24, 0.0657);

total->SetParLimits(25, 1, 10000); 
total->SetParLimits(26, 13.3, 13.4); 
total->SetParLimits(27, 0.16, 0.5); 
total->FixParameter(29, 0.0657);

total->SetParLimits(30, 1, 10000); 
total->SetParLimits(31, 13.45, 13.6); 
total->SetParLimits(32, 0.225, 0.5);   
total->FixParameter(34, 0.0657);

total->SetParLimits(35, 1, 10000); 
total->SetParLimits(36, 13.7, 13.8); 
total->SetParLimits(37, 0.205, 0.5);  
total->FixParameter(39, 0.0657);

total->SetParLimits(40, 380, 1000); 
total->SetParLimits(41, 14, 14.05); 
total->SetParLimits(42, 0.242, 0.5);  
total->FixParameter(44, 0.0657);

total->SetParLimits(45, 50, 1000); 
total->SetParLimits(46, 14.2, 14.35); 
total->SetParLimits(47, 0.18, 0.3);  
...
TF1 *new_bw10 = new TF1("new_bw10", [=](double *x, double *par_total) { 
    return BWModificada(x, par_total, 5, 9); 
}, 11, 15, 5);
