...
}, 11, 15, 4);  // E_min = 11.2, E_max = 12.0, número de parámetros = 4

TF1 *bw2 = new TF1("bw2", [=](double *x, double *par) { 
    return BWModificada(x, par, 1, 1); 
}, 11, 15, 4);  // E_min = 11.5, E_max = 12.5, número de parámetros = 4
...
}, 11, 15, 4);  // E_min = 12.0, E_max = 13.0, número de parámetros = 4

TF1 *bw6 = new TF1("bw4", [=](double *x, double *par) { 
    return BWModificada(x, par, 0, 5); 
}, 15, 15, 4);  // E_min = 12.5, E_max = 14.0, número de parámetros = 4

TF1 *bw7 = new TF1("bw5", [=](double *x, double *par) { 
    return BWModificada(x, par, 0, 6); 
}, 11, 15, 4);  // E_min = 12.8, E_max = 14.4, número de parámetros = 4

TF1 *bw8 = new TF1("bw6", [=](double *x, double *par) { 
    return BWModificada(x, par, 5, 7); 
}, 11, 15, 4);  // E_min = 13.7, E_max = 14.5, número de parámetros = 4
...
bw3->SetParameters(2800, 12, 1, r);
bw4->SetParameters(2800, 12.5, 0.50, r);
bw5->SetParameters(2800, 12.9, 0.20, r);
bw6->SetParameters(2700, 13.3, 0.40, r);
bw7->SetParameters(2000, 13.6, 0.3, r);
bw8->SetParameters(1000, 14.0, 0.3, r);

// Crear la función total (sumatoria de las funciones Breit-Wigner)
TF1 *total = new TF1("mstotal", TotalFunction, 11, 14.7, 32);
...
total->SetParLimits(1, 11.55, 11.65); 
total->SetParLimits(2, 0.150, 0.4);   

total->SetParLimits(5, 11.85, 11.95); 
total->SetParLimits(6, 0.15, 0.4);    
total->FixParameter(7, 8.5e-11);

total->SetParLimits(9, 12, 12.1); 
total->SetParLimits(10, 0.15, 0.4);   
total->FixParameter(11, 8.5e-11);

total->SetParLimits(13, 12.3, 12.65); 
total->SetParLimits(14, 0.15, 1);  
total->FixParameter(15, 8.5e-11);


total->SetParLimits(17, 12.85, 12.95); 
total->SetParLimits(18, 0.15, 0.4);  

total->SetParLimits(21, 13.25, 13.35); 
total->SetParLimits(22, 0.15, 0.4); 

total->SetParLimits(25, 13.55, 13.65); 
total->SetParLimits(26, 0.15, 0.4);   

total->SetParLimits(29, 14.05, 14.15); 
total->SetParLimits(30, 0.15, 0.4);  
