...
// Código para sacar los diferentes Coin Times aplicando cuts al no corregido y al corregido para ver la diferencia.
void readData_coin_times() {
    ...
    TH1F* coinTimeH = new TH1F("coinTimeH","Coincidence Time Comparison: Proton Channel",400,-1000,1000);
...
/*
    TCanvas *c4 = new TCanvas("c4", "Coincidence Time Comparison", 800, 600);
    coinTimeH->SetLineColor(kBlue);
    coinTimeH->Draw();
    correctedCoinTimeH->SetLineColor(kRed);
    correctedCoinTimeH->Draw("SAME");
    coinTimeH->SetStats(0);
    correctedCoinTimeH->SetStats(0);

    
    // Add a legend
    TLegend *legend = new TLegend(0.7, 0.7, 0.9, 0.9);
    legend->AddEntry(coinTimeH, "Original CoinTime", "l");
    legend->AddEntry(correctedCoinTimeH, "Corrected CoinTime", "l");
    legend->Draw();
