...
double BWModificada(double *x, double *par, int l, int bw_index) {
    ...
    // Definir los rangos de energía para cada Breit-Wigner (E_min y E_max)
        ...
        static const double E_max_values[11] = {14.6, 14.6, 14.6, 14.6, 14.6, 14.6, 14.6, 14.6, 14.6, 14.6, 14.6};  // Energías máximas
...
void readData_BW_neutron_penetrability_and_efficiency2()
{
   ...
   for (Long64_t i=0;i<nentries;i++) {
    ...
    if(i % 1000000 == 0){
     ...
     if(coinTime<-10 || coinTime>20) continue; 
      ...
      if (x < -0.95 || x > 0.95 || thetaCM < 10 || thetaCM > 45 || e[detID] < 1)
        continue;
...
}, 11.5, 14.6, 5);  // E_min = 12.0, E_max = 13.0, número de parámetros = 4
...
}, 11.5, 14.6, 5);  // E_min = 12.5, E_max = 14.0, número de parámetros = 4

TF1 *bw9 = new TF1("bw7", [=](double *x, double *par) { 
    return BWModificada(x, par, 0, 7); 
}, 11.5, 14.6, 5);  // E_min = 12.8, E_max = 14.4, número de parámetros = 4

TF1 *bw10 = new TF1("bw8", [=](double *x, double *par) { 
    return BWModificada(x, par, 4, 8); 
}, 11.5, 14.6, 5);  // E_min = 13.7, E_max = 14.5, número de parámetros = 4
...
TF1 *total = new TF1("mstotal", TotalFunction, 11.3, 14.6, 55);
